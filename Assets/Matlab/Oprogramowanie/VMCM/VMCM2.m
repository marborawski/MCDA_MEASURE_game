...
idealne_ = zeros(1,size(dane_norm_wagi2{1},2));
antyidelane_ = zeros(1,size(dane_norm_wagi2{1},2));
for ii = 1:size(dane_norm_wagi2{1},2)
    res = [];
    res = dane_norm_wagi2{rok}(:,ii);
...
%    end
    ...
    if strcmp(charakter(ii),'s') == 1
        idealne_(ii) = tmp(2);
        antyidelane_(ii) = tmp(1);
    else
        idealne_(ii) = tmp(1);
        antyidelane_(ii) = tmp(2);
    end
end

idealne2_ = idealne_ - antyidelane_; 
...
%        S{k + length(dane_norm_wagi2)}(ii) = sum((dane_norm_wagi2{k}(ii,:) - antyidelane).*idealne2)/sum(idealne2.^2);
        S{k}(ii,3) = sum((dane_norm_wagi2{k}(ii,:) - antyidelane_).*idealne2_)/sum(idealne2_.^2);
    end
